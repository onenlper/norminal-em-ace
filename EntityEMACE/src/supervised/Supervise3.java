...
public class Supervise3 {
	...
	public static void train() {
		...
		for (String line : lines) {
			...
			for (CoNLLPart part : d.getParts()) {
				...
				HashSet<String> goldInChain = EMUtil.getGoldInChain(part.getChains());
				...
				for (int i = 0; i < mentions.size(); i++) {
					...
					String fea = createInstance(anaphor, cands, part,
							goldInChain.contains(anaphor.toName()),
							anaFea);
					svmLines.add(fea);
				}
			}
	...
	public static void test() {
		...
		for (String line : lines) {
			...
			for (int k = 0; k < document.getParts().size(); k++) {
				...
				HashSet<String> goldInChain = EMUtil.getGoldInChain(goldPart.getChains());
	...
	public static String createInstance(EntityMention anaphor,
			ArrayList<EntityMention> cands, CoNLLPart part, boolean label,
			AnaphorFea fea) {
		all++;
		if (label) {
			pos += 1;
