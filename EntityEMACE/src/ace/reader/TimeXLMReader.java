...
public class TimeXLMReader extends DefaultHandler {
		...
		public TimeXLMReader(ArrayList<Entity> entities) {
			super();
		...
		public void startElement(String uri, String name, String qName,
				Attributes atts) {
			tags.add(qName);
			if(qName.equalsIgnoreCase("timex2")) {
				entity = new Entity();
				entities.add(entity);
				entity.type = "time";
				entity.subType = "time";
			}
			if(qName.equalsIgnoreCase("timex2_mention")) {
				entityMention = new EntityMention();
				entity.mentions.add(entityMention);
				entityMention.semClass = "time";
				entityMention.subType = "time";
				entityMention.entity = entity;
			}
			if(qName.equalsIgnoreCase("charseq")) {
				if(tags.get(tags.size()-2).equalsIgnoreCase("extent") && 
						tags.get(tags.size()-3).equalsIgnoreCase("timex2_mention") &&
						tags.get(tags.size()-4).equalsIgnoreCase("timex2")) {
