...
import ace.model.EventArgument;
...
public class EventChainReader extends DefaultHandler {
	...
	public void startElement(String uri, String name, String qName, Attributes atts) {
		...
		if (qName.equals("charseq") && tags.get(tags.size() - 2).equals("anchor")
				&& tags.get(tags.size() - 3).equals("event_mention")) {
			currentEventMention.headCharStart = Integer.valueOf(atts.getValue("START"));
			currentEventMention.headCharEnd = Integer.valueOf(atts.getValue("END"));
		}
			...
			try {
				...
				for (EventChain eventChain : eventChains) {
					...
					for (EventMention em : eventChain.getEventMentions()) {
						...
						ems.add(em);
						 if(em.polarity.equalsIgnoreCase("Negative"))
						System.out.println(em.head.replace("\n", nl) + "=>" + em.getLdcScope().replace("\n", nl)
								+ "#" + em.getType() + "#" + em.getSubType() + "@" + em.polarity);
