...
//				ParseTreeEntityMention ptm = new ParseTreeEntityMention();
				ArrayList<EntityMention> goldBoundaryNPEntityMentions = null; 
//						ptm.getEntityMentions(part);

				Collections.sort(goldBoundaryNPEntityMentions);

				ArrayList<EntityMention> anaphorZeros = null; 
//						EMUtil.getAnaphorZeros(part.getChains());
//				ArrayList<EntityMention> anaphorZeros = ZeroDetect.getHeuristicZeros(part);
				// anaphorZeros = zeroDetectTest.detectZeros(part, null);

				ArrayList<EntityMention> candidates = new ArrayList<EntityMention>();
				candidates.addAll(goldBoundaryNPEntityMentions);
...
//		evaluate(corefResults, goldEntities);
	...
	private void findAntecedent(String file, CoNLLPart part,
			HashMap<String, Integer> chainMap, ArrayList<EntityMention> corefResult,
			ArrayList<EntityMention> anaphorZeros, ArrayList<EntityMention> allCandidates) {
...
//							sameSpeaker, context, part);
			...
			if (zero.antecedent != null
				...
				// if(antecedent!=null && antecedent.mType==EntityMentionType.tmporal)
				// {
...
//	public double getMaxEntProb(EntityMention cand, EntityMention pro, boolean sameSpeaker,
...
//	}
	...
	public static boolean findBest(EntityMention zero, ArrayList<EntityMention> cands) {
...
//			ArrayList<EntityMention> goldInChainZeroses = EMUtil
