...
public class MyTreeNode {
	...
	public MyTreeNode getFirstXAncestor(String X) {
		...
		for(int i=ancestors.size()-2;i>=0;i--) {
			...
			if(ancestor.value.equalsIgnoreCase(X)) {
				return ancestor;
			}
		}
		return null;
	...
	public ArrayList<MyTreeNode> getXAncestors(String X) {
		ArrayList<MyTreeNode> ancestors = this.getAncestors();
		ArrayList<MyTreeNode> rets = new ArrayList<MyTreeNode>();
	...
	public MyTreeNode(String value) {
		this.value = value.replace("-LBR-", "(").replace("-RBR-", ")");
	...
	public void addChild(int idx, MyTreeNode node) {
		this.children.add(idx, node);
		node.parent = this;
		this.reorderChildIdx();
	}
	
	private void reorderChildIdx() {
		for(int i=0;i<this.children.size();i++) {
			this.children.get(i).childIndex = i;
		}
	}
	/*
	...
	public ArrayList<MyTreeNode> getLeaves() {
		...
		while (frontiers.size() > 0) {
			...
			if (tn.children.size() == 0 && !tn.emptyCategory) {
				leaves.add(tn);
			}
	...
	private static void loadArabicRules() {
	    ...
	    headRules.put("NX", new String[][]{{"left", "DT","DTNN","DTNNS","DTNNP", "DTNNPS", "DTJJ", "DTNOUN_QUANT", "NOUN_QUANT"}});
	    headRules.put("ADJP", new String[][]{{"rightdis", "ADJ", "DTJJ", "ADJ_NUM", "DTADJ_NUM", "JJR", "DTJJR"}, {"right", "ADJP", "VN", "NOUN", "NNP", "NOFUNC", "NO_FUNC", "NNPS", "NNS", "DTNN", "DTNNS","DTNNP","DTNNPS","DTJJ", "DTNOUN_QUANT", "NOUN_QUANT"}, {"right", "RB", "CD","DTRB","DTCD"}, {"right", "DT"}}); // sometimes right, sometimes left headed??
	    headRules.put("ADVP", new String[][]{{"left", "WRB", "RB", "ADVP", "WHADVP","DTRB"}, {"left", "CD", "RP", "NOUN", "CC", "ADJ", "DTJJ", "ADJ_NUM", "DTADJ_NUM", "IN", "NP", "NNP", "NOFUNC","DTRP","DTNN","DTNNP","DTNNPS","DTNNS","DTJJ", "DTNOUN_QUANT", "NOUN_QUANT"}}); // NNP is a gerund that they called an unknown (=NNP, believe it or not...)
	    headRules.put("CONJP", new String[][]{{"right", "IN", "RB", "NOUN","NNS","NNP", "NNPS", "DTRB", "DTNN", "DTNNS", "DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT"}});
	    headRules.put("FRAG", new String[][]{{"left", "NOUN", "NNPS", "NNP","NNS", "DTNN", "DTNNS", "DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT"}, {"left", "VBP"}});
	    headRules.put("INTJ", new String[][]{{"left", "RP", "UH", "DTRP"}});
	    headRules.put("LST", new String[][]{{"left"}});
	    headRules.put("NAC", new String[][]{{"left", "NP", "SBAR", "PP", "MWP","ADJP", "S", "PRT", "UCP"}, {"left", "ADVP"}}); // note: maybe CC, RB should be the heads?
	    headRules.put("NP", new String[][]{{"left", "NOUN", "DET+NN", "NNS", "NNP", "NNPS", "NP", "PRP", "WHNP", "QP", "WP", "DTNNS", "DTNNPS", "DTNNP", "NOFUNC", "NO_FUNC", "DTNOUN_QUANT", "NOUN_QUANT"}, {"left", "ADJ", "DTJJ", "JJR", "DTJJR", "ADJ_NUM", "DTADJ_NUM"}, {"right", "CD", "DTCD"}, {"left", "PRP$"}, {"right", "DT"}}); // should the JJ rule be left or right?
	    headRules.put("PP", new String[][]{{"left", "PREP", "PP", "MWP","PRT", "X"}, {"left", "NNP", "RP", "NOUN"}, {"left", "NP"}}); // NN is for a mistaken "fy", and many wsT
	    headRules.put("PRN", new String[][]{{"left", "NP"}}); // don't get PUNC
	    headRules.put("PRT", new String[][]{{"left", "RP", "PRT", "IN", "DTRP"}});
	    headRules.put("QP", new String[][]{{"right", "CD", "DTCD", "NOUN", "ADJ", "NNS", "NNP", "NNPS", "DTNN", "DTNNS", "DTNNP", "DTNNPS", "DTJJ", "DTNOUN_QUANT", "NOUN_QUANT"}});

	    headRules.put("S", new String[][]{{"left", "VP", "MWV", "S"}, {"right", "PP", "MWP","ADVP", "SBAR", "UCP", "ADJP"}}); // really important to put in -PRD sensitivity here!
	    headRules.put("SQ", new String[][]{{"left", "VP", "MWV", "PP", "MWP"}}); // to be principled, we need -PRD sensitivity here too.
	    headRules.put("SBAR", new String[][]{{"left", "WHNP", "WHADVP", "WRB", "RP", "IN", "SBAR", "CC", "WP", "WHPP", "ADVP", "PRT", "RB", "X", "DTRB", "DTRP"}, {"left", "NOUN", "NNP", "NNS", "NNPS", "DTNN", "DTNNS", "DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT"}, {"left", "S"}});
	    headRules.put("SBARQ", new String[][]{{"left", "WHNP", "WHADVP", "RP", "IN", "SBAR", "CC", "WP", "WHPP", "ADVP", "PRT", "RB", "X"}, {"left", "NOUN", "NNP", "NNS", "NNPS","DTNN", "DTNNS", "DTNNP", "DTNNPS", "DTNOUN_QUANT", "NOUN_QUANT"}, {"left", "S"}}); // copied from SBAR rule -- look more closely when there's time
	    headRules.put("UCP", new String[][]{{"left"}});
	    headRules.put("VP", new String[][]{{"left", "VBD", "VBN", "VBP", "VBG", "DTVBG", "VN", "DTVN", "VP", "RB", "X","VB"}, {"left", "IN"}, {"left", "NNP","NOFUNC", "NOUN", "DTNN", "DTNNP", "DTNNPS", "DTNNS", "DTNOUN_QUANT", "NOUN_QUANT"}}); // exclude RP because we don't want negation markers as heads -- no useful information?

	    headRules.put("MWV", new String[][]{{"left", "VBD", "VBN", "VBP", "VBG", "DTVBG", "VN", "DTVN", "VP", "RB", "X","VB"}, {"left", "IN"}, {"left", "NNP","NOFUNC", "NOUN", "DTNN", "DTNNP", "DTNNPS", "DTNNS", "DTNOUN_QUANT", "NOUN_QUANT"}}); // exclude RP because we don't want negation markers as heads -- no useful information?
	    headRules.put("MWP", new String[][]{{"left", "PREP", "PP", "MWP","PRT", "X"}, {"left", "NNP", "RP", "NOUN"}, {"left", "NP"}}); // NN is for a mistaken "fy", and many wsT
	...
	public String getTreeBankStyle(boolean leaf) {
		StringBuilder sb = new StringBuilder();
		appendTreeNodeValue(this, sb, leaf);
		return sb.toString();
	}
	
	public int numberMarkChildren() {
		int amount = 0;
