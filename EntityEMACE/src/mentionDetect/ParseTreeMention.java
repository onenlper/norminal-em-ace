...
public class ParseTreeMention extends MentionDetect {
        ...
        public ArrayList<EntityMention> getMentions(CoNLLSentence s) {
                ...
                if (part.getDocument().getLanguage().equalsIgnoreCase("chinese")) {
                        ChineseMention ch = new ChineseMention();
                        return ch.getChineseMention(s);
                } else if (part.getDocument().getLanguage().equalsIgnoreCase("english")) {
                        return getEnglishMention(s);
                }
                return null;
        }
        ...
        private ArrayList<EntityMention> getNamedMention(CoNLLSentence s) {
                ...
                for (Element element : namedEntities) {
                        if (element.content.equalsIgnoreCase("QUANTITY")
                                        || element.content.equalsIgnoreCase("CARDINAL")
                                        || element.content.equalsIgnoreCase("PERCENT")) {
                                continue;
                        }
                        // Mr. Mandela
                        if (element.start > 0
                                        && EnDictionary.getInstance().titles.contains(part
                                                        .getWord(element.start - 1).word)) {
                                continue;
                        }
                        int end = element.end;
                        ...
                        MyTreeNode t1 = mention.s.syntaxTree.leaves
                                        .get(part.getWord(start).indexInSentence);
                        MyTreeNode t2 = mention.s.syntaxTree.leaves
                                        .get(part.getWord(end).indexInSentence);
        ...
        public ArrayList<EntityMention> getAllNounPhrase(CoNLLSentence sentence) {
                ...
                while (frontie.size() > 0) {
                        ...
                        if ((part.lang.equalsIgnoreCase("chi") && (tn.value.toUpperCase()
                                        .startsWith("NP") || tn.value.toUpperCase()
                                        .startsWith("QP")))
                                        || (part.lang.equalsIgnoreCase("eng") && (value
                                                        .startsWith("PRP") || value.startsWith("NP")))) {
        ...
        public ArrayList<EntityMention> getAllNounPhrase(CoNLLPart part) {
                ...
                for (CoNLLSentence sentence : part.getCoNLLSentences()) {
                        ...
                        while (frontie.size() > 0) {
                                MyTreeNode tn = frontie.remove(0);
                                String value = tn.value.toUpperCase();
                                if ((part.lang.equalsIgnoreCase("chi") && (tn.value
                                                .toUpperCase().startsWith("NP") || tn.value
                                                .toUpperCase().startsWith("QP")))
                                                || (part.lang.equalsIgnoreCase("eng") && (value
                                                                .startsWith("PRP") || value.startsWith("NP")))) {
                                        // if (!value.equalsIgnoreCase("NP") &&
                                        // !value.equalsIgnoreCase("PRP")
        ...
        private ArrayList<EntityMention> getNPorPRPMention(CoNLLSentence s) {
                ...
                for (int g = 0; g < npMentions.size(); g++) {
                        ...
                        // System.out.println(start + " " + end + " " + npMention.extent);
        ...
        // public boolean isEnglishAppositive2(Mention ant, Mention mention,
        // CoNLLPart part) {
        // if (ant.s != mention.s) {
        // return false;
        // }
        ...
        // pattens[3] =
        // "/^NP(?:-TMP|-ADV)?$/=m2 < (PRN=m3 < (NP < /^NNS?|CD$/ $-- /^-LRB-$/ $+ /^-RRB-$/))";
        ...
        // int start1 = (int) ((CoreLabel)
        // np2.label()).get(BeginIndexAnnotation.class);
        // int end1 = (int) ((CoreLabel) np2.label()).get(EndIndexAnnotation.class)
        // - 1;
        // int start2 = (int) ((CoreLabel)
        // np3.label()).get(BeginIndexAnnotation.class);
        // int end2 = (int) ((CoreLabel) np3.label()).get(EndIndexAnnotation.class)
        // - 1;
        // if (start1 == ant.start && end1 == ant.end && start2 == mention.start &&
        // end2 == mention.end) {
                        ...
                        for (int j = i+1; j < copyMentions.size(); j++) {
                                ...
                                if (em.headID == em2.headID
                                        ...
                                        if (em.end + 1 < part.getWordCount()
                                                        && (part.getWord(em.end + 1).posTag
                                                                        .equalsIgnoreCase("CC") || part
                                                                        .getWord(em.end + 1).word.equals(","))) {
                                                continue;
                                        }
                                        removes.add(em);
                                        break;
                                ...
                                if (em.headID == em2.headID
                                                && (em.end - em.start > em2.end - em2.start)) {
                                        if (em2.end + 1 < part.getWordCount()
                                                        && (part.getWord(em2.end + 1).posTag
                                                                        .equalsIgnoreCase("CC") || part
                                                                        .getWord(em2.end + 1).word.equals(","))) {
                                                continue;
                                        }
                                        removes.add(em2);
                                        break;
...
//                      if (EnDictionary.getInstance().nonWords.contains(mention.head
//                                      .toLowerCase())) {
//                              removes.add(mention);
//                              continue;
//                      }
//                      // quantRule : not starts with 'any', 'all' etc
//                      if (EnDictionary.getInstance().quantifiers.contains(part
//                                      .getWord(mention.start).word)) {
//                              removes.add(mention);
//                              continue;
//                      }
//                      // partitiveRule
//                      if (mention.start > 2
//                                      && part.getWord(mention.start - 1).word.equals("of")
//                                      && EnDictionary.getInstance().parts.contains(part
//                                                      .getWord(mention.start - 2).word)) {
//                              removes.add(mention);
//                              continue;
//                      }
//                      // bareNPRule
//                      if (part.getWord(mention.headID).posTag.equals("NN")
//                                      && !EnDictionary.getInstance().temporals
//                                                      .contains(mention.head)
//                                      && (mention.end - mention.start == 0 || part
//                                                      .getWord(mention.start).posTag
//                                                      .equalsIgnoreCase("JJ"))) {
//                              removes.add(mention);
//                              continue;
//                      }
//                      // adjective form of nations
//                      if (EnDictionary.getInstance().adjectiveNation
//                                      .contains(mention.extent.toLowerCase())) {
//                              removes.add(mention);
//                              continue;
//                      }
...
//              mentions.addAll(mentionsHash);
        ...
        public static void calEnAttribute(EntityMention em, CoNLLPart part) {
                ...
                if (part.getWord(em.headID).posTag.startsWith("PRP")
                                || (em.start == em.end && em.NE.equalsIgnoreCase("OTHER") && (EnDictionary
                                                .getInstance().allPronouns.contains(em.head
                                                .toLowerCase()) || EnDictionary.getInstance().relativePronouns
                                                .contains(em.head.toLowerCase())))) {
                        em.isPronoun = true;
                } else {
                        em.isPronoun = false;
                }
                ...
                for (int i = em.start; i <= em.end; i++) {
                        if (i != em.headID) {
                                ...
                                if (posTag.equals("NN") || posTag.equals("NR")
                                                || posTag.equals("OD") || posTag.equals("JJ")
                                                || posTag.equals("NT") || posTag.equals("CD")
                                                || posTag.equalsIgnoreCase("RB")) {
                                        if (!EnDictionary.getInstance().stopWords.contains(word
                                                        .getWord())) {
                ...
                if (em.NE.equalsIgnoreCase("person") || em.NE.equalsIgnoreCase("gpe")
                                || em.NE.equalsIgnoreCase("loc")
                                || em.NE.equalsIgnoreCase("org")
                                || em.NE.equalsIgnoreCase("fac")
                                || em.NE.equalsIgnoreCase("law")) {
        ...
        private static void setAnimacy(EntityMention mention, CoNLLPart part) {
                ...
                if (mention.isPronoun) {
                        ...
                        } else if (EnDictionary.getInstance().inanimatePronouns
                                        .contains(headString)) {
                ...
                if (!mention.isPronoun) {
                        if (mention.animacy == Animacy.unknown) {
                                if (EnDictionary.getInstance().animateWords
                                                .contains(headString)) {
                                        mention.animacy = Animacy.animate;
                                } else if (EnDictionary.getInstance().inanimateWords
                                                .contains(headString)) {
                                        mention.animacy = Animacy.unanimate;
                                }
                        }
        ...
        private static void setPerson(EntityMention mention, CoNLLPart part) {
                ...
                if (EnDictionary.getInstance().firstPersonPronouns
                                .contains(mention.extent.toLowerCase())) {
                ...
                } else if (EnDictionary.getInstance().secondPersonPronouns
                                .contains(mention.extent.toLowerCase())) {
                        mention.personEng = PersonEng.YOU;
                } else if (EnDictionary.getInstance().thirdPersonPronouns
                                .contains(mention.extent.toLowerCase())) {
        ...
        private static void setGender(EntityMention mention, CoNLLPart part) {
                ...
                if (mention.isPronoun) {
                        if (EnDictionary.getInstance().malePronouns.contains(mention.extent
                                        .toLowerCase())) {
                                mention.gender = Gender.male;
                        } else if (EnDictionary.getInstance().femalePronouns
                                        .contains(mention.extent.toLowerCase())) {
                        ...
                        if (mention.gender == Gender.unknown) {
                                if (EnDictionary.getInstance().maleWords.contains(headString)) {
                                        mention.gender = Gender.male;
                                } else if (EnDictionary.getInstance().femaleWords
                                                .contains(headString)) {
                                        mention.gender = Gender.female;
                                } else if (EnDictionary.getInstance().neutralWords
                                                .contains(headString)) {
                                        mention.gender = Gender.neuter;
                                }
                        }
                        int genderNumberCount[] = getNumberCount(mention, part);
        ...
        private static int[] getNumberCount(EntityMention mention, CoNLLPart part) {
                ...
                if (part.getWord(headIndex).rawNamedEntity.startsWith("PER")) {
                        ...
                        for (int i = words.size(); i > 0; i--) {
                                if (EnDictionary.getInstance().bigGenderNumber
                                                .containsKey(words.subList(0, i)))
                                        return EnDictionary.getInstance().bigGenderNumber.get(words
                                                        .subList(0, i));
                        }
                }
                return null;
        ...
        public static void setNumber(EntityMention mention, CoNLLPart part) {
                ...
                if (mention.isPronoun) {
                        ...
                        } else if (EnDictionary.getInstance().singularPronouns
                                        .contains(headString)) {
                                mention.number = Number.single;
                        } else {
                                mention.number = Number.single;
                ...
                if (!mention.isPronoun) {
                        if (mention.number == Number.single) {
                                if (EnDictionary.getInstance().singularWords.contains(part
                                                .getWord(mention.headID).orig.toLowerCase())) {
                                        mention.number = Number.single;
                                } else if (EnDictionary.getInstance().pluralPronouns
                                                .contains(part.getWord(mention.headID).orig
                                                                .toLowerCase())) {
                                        mention.number = Number.plural;
                                }
                        }
